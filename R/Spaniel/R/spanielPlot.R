...

{
    ...
    if (!is.null(showFilter)){
        p <- p +
            ggplot2::scale_size_continuous(range=c(ptSizeMax,
                                                   ptSizeMax)) +
            ggplot2::guides(color= ggplot2::guide_legend(),
                            size = ggplot2::guide_legend())
    }
    ...
    if (plotType != "Cluster" & is.null(showFilter)){
        ...
        p <- p + ggplot2::scale_colour_gradientn(
          colours=c("#5f4274", "#5f4274", "#b59c00", "#ffff00")) +
    ...
    if (plotType == "Cluster"){
        p <- p + ggplot2::guides(size=FALSE) +
            ggplot2::scale_size_continuous(range=c(ptSize,
                                                   ptSize))
        ### Customize ##################################################
        n_groups <- length(levels(object@meta.data[["cluster"]]))
        ggColorHue <- function(n, l=65){   # To produce default colors of ggplot2
          hues <- seq(15, 375, length=n+1)
          hcl(h=hues, l=l, c=100)[1:n]
        }
        my_colors <- ggColorHue(n_groups)
        my_colors <- my_colors[unique(sort(object@meta.data[["cluster"]]))]
        p <- p + ggplot2::scale_colour_manual(values=my_colors) +
          theme_void(base_size=20, base_family="Helvetica") +
    ...
    if (plotType == "Cluster"){
        p <- p + ggplot2::guides(size=FALSE) +
            ggplot2::scale_size_continuous(range=c(ptSize,
                                                   ptSize))
        ### Customize ##################################################
        n_groups <- length(levels(object@meta.data[["phase"]]))
        my_colors <- rainbow(n_groups)
        my_colors <- my_colors[unique(sort(object@meta.data[["phase"]]))]
        p <- p + ggplot2::scale_colour_manual(values=my_colors) +
          theme_void(base_size=20, base_family="Helvetica") +
          guides(colour = guide_legend(override.aes = list(size=4)))
        ################################################################
...

{
    ...
    if (plotType == "Cluster"){
        ...
        ### Customize ##################################################
        p <- p + ggplot2::scale_colour_gradientn(
          colours=c("#5f4274", "#5f4274", "#b59c00", "#ffff00", "#ffff00")) +
          theme_void(base_size=20, base_family="Helvetica") +
