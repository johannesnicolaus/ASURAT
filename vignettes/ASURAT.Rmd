...
```{r, message = FALSE}
sce <- TENxPBMCData::TENxPBMCData(dataset = c("pbmc3k"))
...
```{r}
pbmc <- remove_samples(sce = pbmc, min_nReads = 1000, max_nReads = 10000,
...
```{r}
pbmcs$CM <- remove_signs(sce = pbmcs$CM, min_ngenes = 2, max_ngenes = 100)
pbmcs$GO <- remove_signs(sce = pbmcs$GO, min_ngenes = 2, max_ngenes = 100)
pbmcs$KG <- remove_signs(sce = pbmcs$KG, min_ngenes = 2, max_ngenes = 100)
```
...
```{r}
set.seed(1)
res <- Rtsne::Rtsne(t(assay(pbmcs$CM, "counts")), dim = 2, pca = TRUE, initial_dims = 50)
reducedDim(pbmcs$CM, "TSNE") <- res[["Y"]]

set.seed(1)
res <- Rtsne::Rtsne(t(assay(pbmcs$GO, "counts")), dim = 2, pca = TRUE, initial_dims = 20)
reducedDim(pbmcs$GO, "TSNE") <- res[["Y"]]

set.seed(1)
res <- Rtsne::Rtsne(t(assay(pbmcs$KG, "counts")), dim = 3, pca = TRUE, initial_dims = 30)
...
plot_dataframe3D(dataframe3D = dataframe3D,
                 theta = 40, phi = 20, title = "PBMC (KEGG)",
...
dims <- list(seq_len(20), seq_len(20), seq_len(20))
...
plot_dataframe3D(dataframe3D = dataframe3D, labels = labels, colors = NULL,
                 theta = 55, phi = 35, title = "PBMC (KEGG)",
                 xlabel = "tSNE_1", ylabel = "tSNE_2", zlabel = "tSNE_3")
```
...
plot_violin(dataframe1D = dataframe1D, labels = labels, colors = NULL) +
  ...
  ggplot2::theme_classic(base_size = 20) + ggplot2::theme(legend.position = "none")
```
...
plot_violin(dataframe1D = dataframe1D, labels = labels, colors = NULL) +
  ggplot2::labs(title = paste0(vname, "  (B cell marker)"),
                x = "Cluster (CO & MSigDB)", y = "Normalized expression") +
  ggplot2::theme_classic(base_size = 20) + ggplot2::theme(legend.position = "none")
```
...
First, manually curate signs by setting keywords, followed by selecting top
