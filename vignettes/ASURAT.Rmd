...
marker_genes_CB <- dplyr::slice_min(marker_genes_CB, p_val_adj, n = 5)
...
```{r}
labels <- colData(pbmcs$CB)$seurat_clusters
vlist <- list(c("CL:0000798-S", "gamma-delta T cell"),
              c("CL:0000623-V", "natural killer cell"),
              c("GO:0006910-S", "phagocytosis, recognition"),
              c("MSigDBID:12-S", "AIZARANI...B_CELLS..."))
xlabel <- "Cluster (cell type)"
p <- list()
for(i in seq_len(length(vlist))){
  ind <- which(rownames(pbmcs$CB) == vlist[[i]][1])
  subsce <- pbmcs$CB[ind, ]
  ...
  p[[i]] <- ggplot2::ggplot() +
    ...
    ggplot2::labs(title = paste0(vlist[[i]][1], "\n", vlist[[i]][2]),
                  x = xlabel, y = "Sign score") +
