...
pbmcs <- readRDS(file = "~/Desktop/temp.rds")
ident_1 <- c(0)
ident_2 <- c(1,2)
compute_sepI_clusters <- function(
  ...
  for(i in seq_len(dim(subsce)[1])){
    ...
    #--------------------------------------------------
    dist1 <- bubble_sort(list(vec_1, 0))$cnt     # dist(vec_1, (0,...,1)).
    dist2 <- bubble_sort(list(1 - vec_1, 0))$cnt # Dist btw vec_1 and (1,...,0).
...
clusters 2 and 3, use an ASURAT function `compute_sepI_clusters()`, whose
arguments are

* `sce`: SingleCellExperiment object,
* `labels`: a vector of labels of all the samples,
* `nrand_samples`: an integer for the random sampling, and
...
# Appendix. Visualize computational results {#visualization}
